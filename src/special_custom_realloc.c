...
/*   Updated: 2018/01/26 10:33:56 by pfichepo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*special_custom_realloc(void *p, size_t s, t_plage *ic, bool goc)
{
	...
	if (ic && goc)
	{
		...
		{
			page = malloc(s);
			init_page(page, closestsize(s +
				sizeof(t_plage) + sizeof(void*)), true);
			ft_memcpy(page + sizeof(void*), &ic->data + sizeof(void*),
...
void		*special_custom_malloc(size_t size)
{
	...
	{
		g_alc_mng.custom_plage = (t_plage*)ezmmap(closestsize(size
			+ sizeof(t_plage) + sizeof(void*)));
		init_page(g_alc_mng.custom_plage, closestsize(size +
			sizeof(t_plage) + sizeof(void*)), true);
